...
function smokeBuildPlanUsesSavedSnapshotCostsAndDedupedHardware(context) {
    ...
    assert.equal(result.planData.commercial, 14, 'commercial department should be planned as 7% of revenue without VAT');
    ...
    assert.equal(result.planData.planEarned, -44.4, 'plan profit should be recomputed from current plan rows');
}
...
async function smokeTaxesIncludeCharity(context) {
    ...
    assert.equal(planTaxes.commercial, 70, 'plan should include commercial department as отдельную строку 7% от выручки без НДС');
    ...
    await vm.runInContext(`(async () => {
        ...
        await Factual._applyDerivedFacts(
            ...
            { hoursPlastic: 0, hoursTrim: 0, hoursHardware: 0, hoursPackaging: 0 },
            { taxRate: 0.07, vatRate: 0.05, charityRate: 0.01, indirectPerHour: 0 },
            101,
            'Charity Order'
        );
...
async function smokeTaxesFallbackToRevenueWhenImportMissing(context) {
    ...
    assert.equal(fact.fact_revenue, 1000, 'fact revenue should still come from FinTablo import');
    assert.equal(fact.fact_taxes, 70, 'when FinTablo taxes are missing, analytics should fallback to 7% of fact revenue without VAT');
    assert.equal(fact._source_hints.fact_taxes, '7% от факта выручки без НДС');
}
...
async function smokeEnsureComputedOrderUsesCurrentWarehousePlanMaterials(context) {
    ...
    assert.equal(computed.planData.totalCosts, 337, 'total plan cost should include current materials and commercial department on the current base-without-VAT model');
    assert.equal(computed.planData.planEarned, 663, 'plan profit should follow recomputed current rows including commercial department');
}
...
async function smokeStaleSplitImportDoesNotLeakIntoFactMoney(context) {
    ...
    await vm.runInContext(`(async () => {
        App.params = { taxRate: 0.07, vatRate: 0.05, charityRate: 0.01, indirectPerHour: 0 };
        const factData = {
            fact_revenue: 710500,
