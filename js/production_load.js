...
function renderProductionLoadBar(container, load, label, breakdown) {
    ...
    const bookedPct = _clampLoad(roLoadNum(view.scheduledPct), 0, 100);
    // Blue is sold-but-not-yet-done work, beginning precisely after the green fact.
    const remainingPct = _clampLoad(bookedPct - donePct, 0, Math.max(0, 100 - donePct));
    const remainingHours = Math.max(0, roLoadNum(view.booked) - roLoadNum(view.done));
    ...
    container.innerHTML = `<div class="pl-wrap">
      ...
      <div class="pl-trackwrap">
        ...
        <div class="pl-track" aria-label="Зелёный: факт. Синий: оставшиеся часы заказов. Красная отметка: ожидаемый объём к концу сегодняшнего рабочего дня.">
          <div class="pl-seg pl-done" style="width:0%"></div><div class="pl-seg pl-remaining" style="left:${donePct}%;width:0%"></div>${boundaries}
      ...
      <div class="pl-track-note"><span><i class="pl-dot pl-done"></i>факт ${_hrsLoad(view.done)} ч</span><span><i class="pl-dot pl-remaining"></i>осталось по заказам ${_hrsLoad(remainingHours)} ч</span><span><i class="pl-dot pl-expected"></i>ожидалось ${_hrsLoad(view.expected)} ч</span><span>${scope === 'commercial' ? `к цели продать: ещё ${_hrsLoad(Math.max(0, view.plan - view.sold))} ч` : `осталось сделать: ${_hrsLoad(Math.max(0, view.plan - view.done))} ч`}</span></div>
    ...
    const remainingEl = container.querySelector('.pl-seg.pl-remaining');
    const raf = typeof requestAnimationFrame === 'function' ? requestAnimationFrame : fn => fn();
    raf(() => raf(() => {
        if (doneEl) doneEl.style.width = donePct + '%';
