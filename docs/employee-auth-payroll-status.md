...
- Нижняя payroll-таблица в `Часах` теперь группируется по сотруднику и полумесяцу (`1–15`, `16–конец`), чтобы не плодить отдельные строки по дням.
- `Отчеты за сегодня` теперь показывают только производство: management-роль вроде Лёши туда больше не попадает даже при наличии time entries.
- Добавлен one-shot import legacy-часов за `1–15 марта 2026` из ручных Google-таблиц: импортируются `Тая` и `Женя Г`, при этом у Жени пропускаются уже вручную внесенные даты, чтобы не задвоить часы.
