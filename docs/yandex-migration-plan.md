...
2. Добавить Yandex PostgreSQL или другой российский backend как новую базу-кандидат.
3. Импортировать туда snapshot и сверить row counts.
4. Добавить smoke `Supabase -> Yandex parity`: количество строк и контрольные записи совпадают.
5. Включить dual-write для склада: Supabase остается источником истины, Яндекс получает копию.
6. После нескольких дней зеленого мониторинга переключить `calc2` на чтение склада из Яндекса.
7. Повторить по модулям.
