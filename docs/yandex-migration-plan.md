...
> Исторический документ. С релиза единственного production-калькулятора
> `calc.recycleobject.ru` обслуживает приложение, а `calc2.recycleobject.ru`
> оставлен только как совместимый редирект для старых ссылок.
