# Pendant Configurator Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a wizard-modal pendant configurator to the order calculator so managers can quickly assemble letter-pendant orders instead of manually adding each letter as a separate product item.

**Architecture:** New `js/pendant.js` module contains all pendant logic (data model, cost calculation, wizard UI, card rendering). Minimal changes to existing files: button in `index.html`, hooks in `Calculator` object (`app.js`) for save/load/recalculate, pendant rendering in `order-detail.js`. Calculator engine (`calculator.js`) gets one new function.

**Tech Stack:** Vanilla JS, CSS (existing `css/style.css`), Supabase (existing `item_data` JSON blob pattern)

**Spec:** `docs/superpowers/specs/2026-03-16-pendant-configurator-design.md`
...
|------|--------|---------------|
| `js/pendant.js` | **Create** | Pendant module: data model, wizard modal UI, calculator card rendering, warehouse picker integration |
| `js/calculator.js` | **Modify** (add ~40 lines) | `calculatePendantCost()` function, update `calculateOrderSummary`, `calculateProductionLoad`, `calculateFinDirectorData` to accept `pendantItems` |
| `js/app.js` | **Modify** (~60 lines across 6 locations) | `Calculator.pendants[]` array, hooks in `init/resetForm/recalculate/_collectItemsForSave/loadOrder/saveOrder` |
| `index.html` | **Modify** (3 lines) | Add `<script src="js/pendant.js">`, pendant button, pendant container div |
| `js/order-detail.js` | **Modify** (~30 lines) | Render pendant items in the Items tab |
| `css/style.css` | **Modify** (~80 lines) | Pendant wizard modal styles, bead styles, pendant card styles |

---

## Chunk 1: Calculator Engine + Data Model

### Task 1: Add `calculatePendantCost` to calculator.js

**Files:**
- Modify: `js/calculator.js` (after `calculatePackagingCost`, ~line 265)

- [ ] **Step 1: Add `calculatePendantCost` function**

Add after `calculatePackagingCost` (line 265):
...
- [ ] **Step 2: Update `calculateOrderSummary` to include pendants**

In `calculateOrderSummary` (line 491), add `pendantItems` parameter and pendant loop after packaging loop:
...
Add after the `packagingItems` forEach block (after line 524):
...
- [ ] **Step 3: Update `calculateProductionLoad` to include pendants**
...
Add after the `packagingItems` forEach block (after line 364):
...
- [ ] **Step 4: Update `calculateFinDirectorData` to include pendants**
...
Add after the packaging forEach block (after line 464):
...
```js
    ...
    (pendantItems || []).forEach(pnd => {
        ...
        // Elements → also hardware purchase (element_price_per_unit is the purchase/cost price, not sell)
        // Note: element_price is a sell-level price from blanks, so we approximate cost as price * (1 - margin)
        // For FinDirector purposes, we treat element purchases as hardware since they are bought components
...
git commit -m "feat: add calculatePendantCost and pendant support in order summary/production/fin functions"
```

---

### Task 2: Add pendant hooks to Calculator object in app.js

**Files:**
- Modify: `js/app.js` (6 locations)

- [ ] **Step 1: Add `pendants` array to Calculator object**

At line 722, after `packagingItems: [],` add:

```js
    pendants: [],       // Pendant items (unlimited)
```

- [ ] **Step 2: Update `resetForm` to clear pendants**

In `resetForm()` method, after `this.packagingItems = [];` add:

```js
        this.pendants = [];
```

Also add after the packaging list clear:
```js
        const pendantList = document.getElementById('calc-pendants-container');
        if (pendantList) pendantList.innerHTML = '';
```

- [ ] **Step 3: Update `_doRecalculate` to calculate pendant costs**

In `_doRecalculate(params)` (line 2763), after `this.packagingItems.forEach(pkg => { pkg.result = calculatePackagingCost(pkg, params); });` add:
...
Then update the two calls to `calculateProductionLoad` and `calculateOrderSummary` in `_doRecalculate` to pass `this.pendants`:
...
Same for `calculateOrderSummary` and `calculateFinDirectorData` — add `this.pendants` as last argument.

- [ ] **Step 4: Update `saveOrder` and `_collectItemsForSave` for pendants**

In `saveOrder()` (~line 3758), update the two calls to `calculateProductionLoad` and `calculateOrderSummary` to pass `this.pendants` as well.

In `_collectItemsForSave()` (~line 3634), after the extra costs block (line 3738), add:
...
- [ ] **Step 5: Update `loadOrder` to restore pendants**

In `loadOrder()` (~line 4199), after the extra costs restore block, add:
...
```js
        ...
        if (typeof Pendant !== 'undefined') {
            this.pendants.forEach((pnd, i) => Pendant.renderCard(i));
        }
```
...
```bash
git add js/app.js
git commit -m "feat: add pendant array to Calculator with save/load/recalculate hooks"
```

---

## Chunk 2: Pendant Module (UI + Wizard)

### Task 3: Add pendant container and button to index.html

**Files:**
- Modify: `index.html`

- [ ] **Step 1: Add script tag for pendant.js**

Find the last `<script>` tag in `index.html` (before `</body>`) and add:
```html
<script src="js/pendant.js"></script>
```

- [ ] **Step 2: Add pendant section between items and hardware**

After the add-item button div (line 283) and before the hardware section (line 285), add:
...
```bash
git add index.html
git commit -m "feat: add pendant container and button to calculator HTML"
```

---

### Task 4: Create pendant.js — data model and card rendering

**Files:**
- Create: `js/pendant.js`

- [ ] **Step 1: Create pendant.js with Pendant module skeleton**
...
```bash
git add js/pendant.js
git commit -m "feat: create pendant.js with wizard modal and card rendering"
```

---

### Task 5: Add pendant CSS styles

**Files:**
- Modify: `css/style.css` (append at end)

- [ ] **Step 1: Add pendant styles**
...
.pendant-wizard-header h2 {
    margin: 0;
    font-size: 18px;
}

.pendant-wizard-steps {
    display: flex;
    gap: 4px;
    padding: 12px 20px;
    border-bottom: 1px solid var(--border);
...
.pendant-step-btn.active {
    background: var(--accent);
    color: #fff;
    border-color: var(--accent);
}

.pendant-step-btn.done {
    border-color: var(--green);
    color: var(--green);
}

.pendant-step-btn:disabled {
    opacity: 0.4;
    cursor: not-allowed;
}
...
.pendant-beads-selectable .pendant-bead {
    cursor: pointer;
}

.pendant-beads-selectable .pendant-bead:hover {
    border-color: var(--accent);
}

.pendant-bead.selected {
    border-color: var(--accent);
...
.pendant-summary-table {
    width: 100%;
    border-collapse: collapse;
    font-size: 13px;
}

.pendant-summary-table td {
    padding: 6px 8px;
    border-bottom: 1px solid var(--border);
}

.pendant-summary-table .pendant-summary-header td {
    font-size: 11px;
    color: var(--text-muted);
    font-weight: 600;
...
.pendant-summary-table .pendant-summary-total td {
    border-top: 2px solid var(--text);
    border-bottom: none;
}

.pendant-wh-item:hover {
    background: var(--accent-light);
}
```
...
git commit -m "feat: add pendant wizard and card CSS styles"
```
...
### Task 6: Render pendants in order-detail.js

**Files:**
- Modify: `js/order-detail.js`

- [ ] **Step 1: Add pendant rendering to renderItemsTab**

In `renderItemsTab()`, after filtering products/hardware/packaging (line 205), add:

```js
        const pendantItems = this.currentItems.filter(i => i.item_type === 'pendant');
```

Then add a helper to render all pendants as HTML:
...
In **both** the grouped (`if (hasSetNames)`) and non-grouped (`else`) rendering paths, add pendant rendering before the hardware section:
...
This ensures pendants appear regardless of whether other items use marketplace set grouping.

- [ ] **Step 2: Add `_renderPendantDetail` method**

Add to the OrderDetail object:
...
```bash
git add js/order-detail.js
git commit -m "feat: render pendant details in order-detail items tab"
```

---

### Task 7: Final integration — wire up all calling sites

**Files:**
- Modify: `js/app.js` (update remaining `calculateOrderSummary` / `calculateProductionLoad` / `calculateFinDirectorData` call sites)

- [ ] **Step 1: Find and update all call sites**

Search `js/app.js` for all calls to these 3 functions and add `this.pendants` (or `Calculator.pendants`) as the last argument:

Call sites to update:
- `_doRecalculate` (already done in Task 2 step 3)
- `saveOrder()` (~line 3758-3759)
- Any autosave function that calls them
- `factual.js` or `production_plan.js` if they call these functions (they call from order data, so they pass stored values — no change needed)
...
For `autosave` at **lines 3572-3573** of `app.js` (the explicit calls inside the autosave function):
```js
const load = calculateProductionLoad(this.items, this.hardwareItems, this.packagingItems, App.params, this.pendants);
const summary = calculateOrderSummary(this.items, this.hardwareItems, this.packagingItems, this.extraCosts, App.params, this.pendants);
```

**Important:** There are exactly 3 call sites to update in app.js:
1. `_doRecalculate` (~line 2919/2955)
2. `saveOrder` (~line 3758-3759)
3. `autosave` (~line 3572-3573)

- [ ] **Step 2: Update `_doRecalculate` summary rendering for pendants**

In `_doRecalculate`, after pendant card rendering, add pendant re-render:
...
```bash
git add js/app.js
git commit -m "feat: wire up pendant support in all calculator call sites"
```
...
- [ ] **Step 1: Open the calculator and verify the pendant button appears**

Expected: «+ Подвес из букв» button visible between items and hardware sections.

- [ ] **Step 2: Create a pendant via the wizard**

1. Click «+ Подвес из букв»
2. Enter quantity: 500, inscription: "КЭШШШ❤️"
3. Step 2: Select К, Э → assign "белый". Select Ш, Ш, Ш → assign "голубой". Select ❤️ → assign "красный"
4. Step 3: Check print on ❤️, enter price 15.50
5. Step 4: Pick cord from warehouse, pick carabiner from warehouse
6. Step 5: Verify summary table, prices auto-calculated
7. Click «Добавить в заказ»

Expected: Pendant card appears in calculator with cost/price/margin.

- [ ] **Step 3: Save the order and reload**

1. Save the order
2. Go to orders list, open the same order
3. Verify pendant is restored correctly
4. Edit the pendant — verify wizard opens with existing data

Expected: Data persists through save/load cycle.

- [ ] **Step 4: Check order-detail view**

Open the order in the detail view (read-only). Verify pendant section shows grouped items table.

- [ ] **Step 5: Verify existing functionality is not broken**

1. Create a regular order with product items, hardware, packaging
2. Save and reload
...
```bash
git add -A
git commit -m "feat: pendant configurator — complete implementation"
```
