...
A: Да и нужно. Файлы вроде `js/warehouse.js`, `js/orders.js`, `js/calculator.js` содержат ВСЮ бизнес-логику. Цель — перенести её 1:1, а не переизобретать. Используй grep, читай, копируй формулы.

**Q: Что делать если нашёл баг в старой логике?**
A: Перенеси как есть, добавь TODO-комментарий, не фикси по ходу миграции. Любая правка логики = риск регрессии. Багфиксы — после Stage D.

**Q: Структура таблицы в Supabase сложная — упростить?**
A: Нет. Перенести структуру 1:1 (можно убрать только `item_data` JSON в warehouse_items — разворачиваем в нормальные колонки, как сказано в спеке). Любая нормализация — после Stage D.
