...
test('_isSoldOrder: отменённые и неоплаченные черновики не в счёт', () => {
    ...
    assert.equal(_isSoldOrder({ status: 'production_casting', production_purpose: 'rework' }), false);
    assert.equal(_isSoldOrder({ status: 'sample', production_purpose: 'stock_sample' }), false);
});
...
test('collectQuarterLoad: переделка и сток занимают мощность, но не закрывают продажи', () => {
    const now = new Date(2026, 7, 19);
    const orders = [
        { id: 1, order_name: 'Клиентский заказ', status: 'completed', deadline_start: '2026-08-05', total_hours_plan: 40 },
        { id: 2, order_name: 'Переделка брака', status: 'production_casting', production_purpose: 'rework', deadline_start: '2026-08-06', total_hours_plan: 3.04, total_cost_plan: 14604.5 },
        { id: 3, order_name: 'Сток-образцы', status: 'sample', production_purpose: 'stock_sample', deadline_start: '2026-08-07', total_hours_plan: 29, total_cost_plan: 3200 },
    ];
    const settings = { seasonal_load_plan_json: JSON.stringify({ Q3: 100 }) };
    const { load, breakdown } = collectQuarterLoad(orders, [], settings, now);

    assert.equal(load.sold, 40, 'в план продаж попадает только клиентский заказ');
    assert.equal(load.gap, 60, 'внутренние работы не уменьшают, сколько ещё надо продать');
    assert.equal(load.nonCommercial, 32.04);
    assert.equal(load.booked, 72.04, 'цех реально занят продажами + внутренними работами');
    assert.equal(Math.round(load.freeCapacity * 100) / 100, 27.96);
    assert.equal(breakdown.nonCommercialRows.length, 2);
    assert.equal(breakdown.nonCommercialLoss, 17804.5);
});
